...
| 3 | Antidepressivos | ISRS, ISRSN, tricíclicos, atípicos, IMAO, SARI (perfil + Ki + PK) | 🟡 perfis mockData ok; prosa moleculeDetails pendente |
...
Pendente deste bloco (sub-parte 3b): a **prosa clínica** dos antidepressivos em `moleculeDetails.ts` (psychiatryUse, usos on/off-label) ainda não foi revisada — é uma função com ramos por molécula, será feita numa próxima passada. Os valores numéricos de Ki ficam para o Bloco 9 (auditoria numérica).
