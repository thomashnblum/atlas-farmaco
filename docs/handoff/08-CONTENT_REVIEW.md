...
### Bloco 8 — Transtornos & tratamentos — 2026-07-11
**Importante:** transtornos e tratamentos NÃO estão no `mockData` — vivem no **Supabase** (o `dataService` inicia vazio e puxa da nuvem; seed via `scripts/insert_treatments.js` + batches). A revisão foi feita consultando o **banco ao vivo** (17 transtornos, 102 linhas de tratamento).

**Perfis clínicos dos transtornos** (`DISORDER_CLINICAL_PROFILES`, 17): revisados — neurobiologia/epidemiologia/marcadores **corretos** (ex.: hiper DA mesolímbica + hipo mesocortical + hipofunção NMDA na esquizofrenia; atrofia hipocampal/BDNF na TDM; circuito CSTC no TOC). Sem erros graves.

**Mapeamentos de tratamento (linhas):** revisados os 102 — **clinicamente sólidos e bem organizados por linha.** Conferidos e corretos: TDM (ISRS/duais 1ª linha; TCA 2ª; lítio/quetiapina/aripiprazol adjuvante; IMAO/cetamina refratária), TOC (ISRS alta dose + clomipramina 2ª + antipsicótico adjuvante), esquizofrenia (atípicos 1ª, haloperidol 2ª, clozapina refratária), TAG, pânico, TEPT, bipolar I/II, TDAH.

**4 imprecisões menores registradas — NÃO alteradas (dado é produção; aguardando decisão do Thom):**
1. **Lisdexanfetamina como off-label na Bulimia** — é aprovada para **Transtorno de Compulsão Alimentar Periódica (TCAP)**, não bulimia; em BN há preocupação de abuso/purgação. Candidata nº 1 a **remover** do mapeamento de BN. (Recomendação de segurança.)
2. **Diazepam como "1ª Linha" no Transtorno por Uso de Álcool** — é 1ª linha para a **abstinência/detox**, não para o tratamento de manutenção do transtorno. Rótulo poderia virar "Adjuvante" ou nota "(abstinência)".
